...
async function render(
...

  {
    ...
    // TODO(@REDACTED, 2022/08/10): readyToPlay not detected properly on Firefox Mobile,
  ...
  if (configuration['insert_help']) {
    // TODO(@REDACTED, 2022/09/30): add `'Take Help Tour'` option toggle on splash screen
    ...
    if (
      ...
      // TODO(@REDACTED, 2022/09/30): create PIANOTO help-tour
      // helpTour = new NonotoTour(
      //   [configuration['main_language']],
      //   piaInpainterGraphicalView,
      //   REGISTER_IDLE_STATE_DETECTOR ? 2 * 1000 * 60 : undefined
      ...
      // FIXME(@REDACTED, 2021/10/14): else branch should not be required,
...
//HACK(@REDACTED, 2022/10/20): NO-OP, just using this to introduce an HMR dependency link from
