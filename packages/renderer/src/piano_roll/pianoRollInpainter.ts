...
// TODO(@REDACTED, 2022/08/10): clean this up
...
> {
    ...
    const handleNewNotes: (
      ...
      let incomingNotes = newNotesPia
        ...
        .map((note) => ({
          ...note,
          instrument: 0,
          // HACK(@REDACTED, 2022/09/13): distinguish generated notes from loaded ones by setting program to 1
          program: 1,
        }))
      if (isFirstRequest) {
        // HACK(@REDACTED, 2022/09/02): patches a weird behaviour in the PIA API
      ...
      const isDone =
        responseJSON.done ||
        // TODO(@REDACTED, 2022/09/09): move this request validity check higher in the callchain
        Math.abs(
  ...
  protected addNote(
    ...
    // TODO(@REDACTED, 2022/09/02): not very efficient, should simply insert at proper index
  ...
  // TODO(@REDACTED, 2022/09/29): create a proper status checking method on the API
