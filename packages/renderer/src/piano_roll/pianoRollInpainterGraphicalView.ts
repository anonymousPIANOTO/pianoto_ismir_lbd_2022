...
> {
  ...
  protected drawNewNote(
    ...
    // HACK(@REDACTED, 2022/09/08): remove this manual, inline update
  ...
  protected async regenerationCallback(): Promise<void> {
    if (
      ...
      // TODO(@REDACTED, 2022/08/10): setup a looser way of handling concurrent requests?
  ...
  protected _refresh(): void {
    ...
    this.abortDrawing()
    // HACK(@REDACTED, 2022/08/10): this seems hackish to need to do this,
    ...
    if (data.newNotes != null) {
      ...
      this.drawNewNotes(data.newNotes.notes).finally(() => {
        if (!data.partialUpdate) {
          this.emit('ready')
          // HACK(@REDACTED, 2022/08/10): feels hackish to trigger 'ready' manually here
