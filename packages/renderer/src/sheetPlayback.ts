...
> extends PlaybackManager<InpainterT> {
  ...
  // TODO(@REDACTED): set-up chords instrument simply as an additional MIDI channel
  ...
  constructor(
    ...
    if (this.inpainter.forceDuration_ticks != null) {
      ...
      // FIXME(@REDACTED, 2022/09/22): clean this up!!
  ...
  protected onInpainterChange(data: {
    ...
    // FIXME(@REDACTED, 2022/07/31): Fix this!! breaks NONOTO looping
    const totalDuration =
  ...
  protected switchTracks(): void {
    // TODO(@REDACTED, 2022/04/29): add crossfade for smooth transitioning
  ...
  protected scheduleNoteToInstrument(
    ...
    if (part == undefined) {
      ...
      // @HACK(@REDACTED): offsetting the lookAhead to ensure perfect synchronization when
  ...
  scheduleChordsPlayer(
    ...
    // FIXME(@REDACTED): assumes a TimeSignature of 4/4
