...
// TODO(@REDACTED, 2021/11/04): merge all symbolic modes
...
// TODO(@REDACTED, 2021/10/15): move this to index.ts
// TODO(@REDACTED, 2021/10/15): remove 'link_channel_prefix' from the editable configuration,
...
export class SplashScreen {
  ...
  constructor(
    ...
    if (this.insertCustomAPIAdressInput) {
      ...
      serverSetupHelpIcon.href =
        'https://github.com/anonymousPIANOTO/pianoto_ismir_lbd_2022#running-the-model-locally'
    ...
    // TODO(@REDACTED, 2021/11/04): should we validate the provided available
    // application modes?
    ...
    if (this.insertEulaAccept) {
      // TODO(@REDACTED, 2022/04/25): do not return elements, attach them directly
      //  as properties?
  ...
  protected getSelectedApplicationMode(): ApplicationMode {
    ...
    if (
      ...
      // HACK(@REDACTED, 2022/09/15): attempt at a workaround for the Chrome Mobile soft reload bug
