...
function makeOpenGraphData(): HtmlTagDescriptor[] {
  ...
  if (VITE_APP_TITLE.toLowerCase() == 'notono') {
    tags = new Map([
      ...
      [
        'og:description',
        'Let your visual thinking draw new sounds. An AI-based interface for inpainting of musical sounds, developed at REDACTED.',
      ],
    ])
    ...
    tags = new Map([
      ...
      [
        'og:description',
        'Easily create polyphonic music with just the tip of your finger. Developed at REDACTED.',
      ],
    ])
    if (VITE_DEPLOYMENT_URL != undefined) {
      tags.set(
    ...
    tags = new Map([
      ...
      [
        'og:description',
        'Be the piano maestro that you deserve to be! Developed at REDACTED.',
      ],
    ])
  } else if (
  ...
  if (
    // TODO(@REDACTED, 2022/09/30): create preview with all three interfaces across three columns for packaged-deploy
    VITE_APP_TITLE.toLowerCase().replace('.', '-') == 'music-inpainting-ts' ||
